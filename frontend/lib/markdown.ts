...
export function reportToMarkdown(r: Report): string {
  const lines: string[] = [];
  lines.push(`# Kintsugi — ${r.dataset_name}`);
  lines.push("");
  ...
  lines.push("---");
  lines.push(`_Generated by Kintsugi${r.ai_available ? "" : " (deterministic mode)"} at ${r.generated_at}_`);
  return lines.join("\n");
}
...
export function downloadMarkdown(r: Report) {
  ...
  a.href = url;
  a.download = `kintsugi-${r.dataset_name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.md`;
