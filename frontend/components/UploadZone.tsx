    ...
    <div className="space-y-5">
      ...
      >
        ...
        <p className="mt-3 font-medium text-foreground">
          {file ? file.name : isDragActive ? "Drop your CSV here" : "Drag & drop a CSV, or click to browse"}
        </p>
        <p className="mt-1 text-sm text-muted-foreground">Up to 10 MB · processed in-memory, never stored</p>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
        <div className="flex-1">
          <label htmlFor="target" className="mb-1.5 block text-sm font-medium text-foreground">
            Target column <span className="font-normal text-muted-foreground">(optional — enables leakage & imbalance checks)</span>
          </label>
          <input
            id="target"
            value={target}
      ...
      <div className="rounded-2xl bg-muted/50 p-4">
        <p className="mb-3 text-sm font-medium text-muted-foreground">No CSV handy? Try a sample dataset:</p>
        <div className="grid gap-2 sm:grid-cols-3">
          {demos.map((d) => (
            <button
              ...
              className="group rounded-xl border bg-card p-3 text-left transition hover:border-primary/50 hover:shadow-sm disabled:opacity-60"
            >
